...
% layers, architecture being a n x 1 vector of layer sizes e.g. [784 100 10]
    ...
    nn.dropConnectFraction              = 0;            %  Dropconnect level (Li Wan et all 2013)
    nn.testing                          = 0;            %  Internal variable. nntest sets this to one.
    nn.output                           = 'sigm';       %  output unit 'sigm' (=logistic), 'softmax' and 'linear'
    nn.dcTestSamples                    = 0;            %  Number of samples to draw from when using a dropconnect model for inference
    
    for i = 2 : nn.n   
        % weights and weight momentum
        nn.W{i - 1} = (rand(nn.size(i), nn.size(i - 1)+1) - 0.5) * 8 * sqrt(6 / (nn.size(i) + nn.size(i - 1)));
